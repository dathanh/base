...
class Utils
{
    ...
    public static function useTables($obj, $tableNames = [])
    {
        ...
        foreach ($tableNames as $tableName) {
            ...
            if (isset(BaseController::$_globalObjects['tables'][$tableClass]) && ! empty(BaseController::$_globalObjects['tables'][$tableClass])) {
                $obj->$tableName = BaseController::$_globalObjects['tables'][$tableClass];
            } else {
                $table = TableRegistry::get($tableName);
                if (!empty($table)) {
                    $obj->$tableClass = BaseController::$_globalObjects['tables'][$tableClass] = $table;
                }
            }
    ...
    public static function useComponents($obj, $componentNames = [])
    {
        ...
        foreach ($componentNames as $componentName) {
            ...
            $controller = BaseController::$_instance;
            if (empty($controller)) {
                $controller = new AppController(new \Cake\Network\Request(), new \Cake\Network\Response());
                $controller->initialize();
            }
            if (isset(BaseController::$_globalObjects['components'][$componentName]) && !empty(BaseController::$_globalObjects['components'][$componentName])) {
                $obj->$componentName = BaseController::$_globalObjects['components'][$componentName];
            } else {
                $pluginName = 'App';
                ...
                if (class_exists($componentClass)) {
                    ...
                    BaseController::$_globalObjects['components'][$componentName] = $obj->$componentName = $controller->$componentName;
                }
            }
    ...
    public static function cacheExistedComponents($obj)
    {
        if (! isset($obj->hasLoadComponents) && isset($obj->components) && is_array($obj->components) && ! empty($obj->components)) {
            foreach ($obj->components as $key => $val) {
                ...
                if (is_string($componentName) && ! empty($componentName) && isset($obj->$componentName) && ! isset(BaseController::$_globalObjects['components'][$componentName])) {
                    BaseController::$_globalObjects['components'][$componentName] = $obj->$componentName;
                }
            }
