<?php
$this->layout = false;
$this->Form->setTemplates([
    'inputContainer' => '<div class="form-group row d-flex align-items-center ld -12">{{content}}</div>',
    'label' => '<label class="col-lg-4 form-control-label d-flex justify-content-lg-end" {{attrs}}>{{text}}</label>',
    'input' => ' <div class="col-lg-12"><input type="{{type}}"  class="form-control"  name="{{name}}" {{attrs}}/></div>',
]);
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <title><?= $title ?></title>
        <meta name="description" content="Elisyam is a Web App and Admin Dashboard Template built with Bootstrap 4">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <!-- Google Fonts -->
        <script src="https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js"></script>
        <script>
            WebFont.load({
                google: {"families": ["Montserrat:400,500,600,700", "Noto+Sans:400,700"]},
                active: function () {
                    sessionStorage.fonts = true;
                }
            });
        </script>
        <!-- Favicon -->
        <link rel="apple-touch-icon" sizes="180x180" href="/backend/assets/img/apple-touch-icon.png">
        <link rel="icon" type="image/png" sizes="32x32" href="/backend/assets/img/favicon-32x32.png">
        <link rel="icon" type="image/png" sizes="16x16" href="/backend/assets/img/favicon-16x16.png">
        <!-- Stylesheet -->
        <link rel="stylesheet" href="/backend/assets/vendors/css/base/bootstrap.min.css">
        <link rel="stylesheet" href="/backend/assets/vendors/css/base/elisyam-1.5.min.css">
        <link rel="stylesheet" href="/backend/assets/css/animate/animate.min.css">
        <!-- Tweaks for older IEs--><!--[if lt IE 9]>
        <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
        <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script><![endif]-->
    </head>
    <body class="bg-white">
        <!-- Begin Preloader -->
        <div id="preloader">
            <div class="canvas">
                <img src="/backend/assets/img/logo.gif.png" alt="logo" class="loader-logo">
                <div class="spinner"></div>   
            </div>
        </div>
        <!-- End Preloader -->
        <!-- Begin Container -->
        <div class="container-fluid no-padding h-100">
            <div class="row flex-row h-100 bg-white">
                <!-- Begin Left Content -->
                <div class="col-xl-3 col-lg-5 col-md-5 col-sm-12 col-12 no-padding">
                    <div class="elisyam-bg background-03">
                        <div class="elisyam-overlay overlay-08"></div>
                        <div class="authentication-col-content-2 mx-auto text-center">
                            <div class="logo-centered">
                                <a href="<?= $this->Url->build('/backend', true) ?>">
                                    <img src="/backend/assets/img/logo.gif.png" alt="logo">
                                </a>
                            </div>
                            <h1>Join Our Community</h1>
                        </div>
                    </div>
                </div>
                <!-- End Left Content -->
                <!-- Begin Right Content -->
                <div class="col-xl-9 col-lg-7 col-md-7 col-sm-12 col-12 my-auto no-padding">
                    <!-- Begin Form -->
                    <div class="authentication-form-2 mx-auto">
                        <div class="tab-content" id="animate-tab-content">
                            <!-- Begin Sign In -->
                            <div role="tabpanel" class="tab-pane show active" id="singin" aria-labelledby="singin-tab">
                                <h3>Sign In To Admin</h3>
                                <?= $this->Form->create('AdminUser', ['method' => 'post', 'class' => 'm-t']) ?>
                                <div class="group material-input">
                                    <?= $this->Form->control('email', ['label' => false, 'type' => 'email', 'class' => 'form-control', 'placeholder' => __('Email'), 'required' => true]) ?>
                                </div>
                                <div class="group material-input">
                                    <?= $this->Form->control('password', ['label' => false, 'type' => 'password', 'class' => 'form-control', 'placeholder' => __('Password'), 'required' => true]) ?>
                                </div>
                                <div class="sign-btn text-center">
                                    <?= $this->Form->button(__('Login'), ['type' => 'submit', 'class' => 'btn btn-lg btn-gradient-01']) ?>
                                </div>
                                <?= $this->Form->end() ?>
                            </div>
                            <!-- End Sign In -->
                        </div>
                    </div>
                    <!-- End Form -->                        
                </div>
                <!-- End Right Content -->
            </div>
            <!-- End Row -->
            <?= $this->Flash->render() ?>
        </div>
        <!-- End Container -->    
        <!-- Begin Vendor Js -->
        <script src="/backend/assets/vendors/js/base/jquery.min.js"></script>
        <script src="/backend/assets/vendors/js/base/core.min.js"></script>
        <!-- End Vendor Js -->
        <script src="/backend/assets/vendors/js/nicescroll/nicescroll.min.js"></script>
        <script src="/backend/assets/vendors/js/noty/noty.min.js"></script>
        <!-- Begin Page Vendor Js -->
        <script src="/backend/assets/vendors/js/app/app.min.js"></script>
        <!-- End Page Vendor Js -->
        <!-- Begin Page Snippets -->
        <script src="/backend/js/common.js"></script>
        <!-- End Page Snippets -->
    </body>
</html>